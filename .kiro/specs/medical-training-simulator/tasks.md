...
- [ ] 3.1 OpenAI API統合の基盤実装
  - OpenAI APIクライアントを設定し、基本的なLLM通信機能を実装
  - エラーハンドリング、レート制限、リトライロジックを含む
  - _要件: 1.3, 2.4_

- [ ] 3.2 患者ペルソナ生成機能の実装
  - 診療科、難易度に基づいた患者生成プロンプトを作成
  - LLMを使用して基本的な患者ペルソナを生成する機能を実装
  - 生成された患者データの検証とサニタイゼーション機能を追加
  - PatientPersona型とバリデーション機能を活用した実装
  - _要件: 1.2, 1.3, 1.5, 1.6_

### 4. 患者選択とセットアップUIの実装
- [ ] 4.1 患者生成設定インターフェースの作成
  - 診療科選択、難易度選択のシンプルなUIコンポーネントを実装
  - 外来モードのみをサポート（入院モードは後のフェーズで実装）
  - フォームバリデーションとユーザーフィードバック機能を実装
  - _要件: 1.1, 1.2, 1.3_

- [ ] 4.2 患者生成とローディング機能の実装
  - 患者生成APIエンドポイントを作成
  - ローディング状態とプログレス表示を実装
  - 生成エラーハンドリングと再試行機能を追加
  - _要件: 1.5, 1.6_
...
### 次の実装フェーズの準備状況
- ✅ **Zustand状態管理実装** - 型定義・バリデーション機能完了により実装準備完了
